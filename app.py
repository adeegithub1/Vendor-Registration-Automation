...
def run_full_pipeline(questionnaire_path: Path) -> None:
    """
    Phase 7 pipeline: the FULL end-to-end orchestration, from the original
    questionnaire to the completed, filled document.

    THIS FUNCTION IS PURE ORCHESTRATION. It introduces no new business
    logic, no new classes, and no new error-handling behavior beyond what
    already exists in each phase it calls. It simply calls the five
    existing pipeline functions, in order, exactly as they already work
    when run individually via their own CLI flags:

        1. run_docx_question_detection(questionnaire_path)  -- Phase 2
        2. run_knowledge_extraction()                         -- Phase 3
        3. run_question_classification()                       -- Phase 4
        4. run_answer_generation()                               -- Phase 5
        5. run_fill_questionnaire(questionnaire_path)             -- Phase 6

    Each step reads its input from the same temp/*.json files the
    individual phases already read from and write to -- this is the
    project's existing file-based pipeline architecture, unchanged here.

    ERROR HANDLING (already provided by the modules each step calls,
    NOT reimplemented here):
        - One bad company document -> DocumentManager (Phase 3) logs it
          and continues; does not stop the batch.
        - One failed AI request -> AIService (Phase 5) retries per
          settings.ai_max_retries, then AnswerGenerationEngine records
          NOT FOUND and continues to the next question.
        - One unanswerable/unresolvable question -> AnswerGenerationEngine
          and WordQuestionnaireFiller (Phases 5-6) write NOT FOUND and
          continue; one question never stops the batch.
        - An unrecoverable, whole-pipeline problem (e.g. the questionnaire
          file itself cannot be opened at all) -> the relevant existing
          step already exits with a clear error message, exactly as it
          does when that step is run on its own. This orchestrator does
          not swallow or alter that behavior.

    Args:
        questionnaire_path: Path to the ORIGINAL vendor questionnaire
            .docx file. Used for both question detection (Phase 2) and
            filling (Phase 6), which must be the same file so recorded
            table/row/cell locations line up correctly.
    """
    print("\n" + "#" * 60)
    print("# VENDOR QUESTIONNAIRE AI - FULL PIPELINE (PHASE 7)")
    print("#" * 60)
    print(f"# Questionnaire : {questionnaire_path}")
    print(f"# AI provider   : {settings.ai_provider}")
    print("#" * 60)

    run_docx_question_detection(questionnaire_path)
    run_knowledge_extraction()
    run_question_classification()
    run_answer_generation()
    run_fill_questionnaire(questionnaire_path)

    print("\n" + "#" * 60)
    print("# FULL PIPELINE COMPLETE")
    print(f"# Filled questionnaire: {settings.filled_questionnaire_path}")
    print("#" * 60 + "\n")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="PATH",
        help="Run Phase 7: the FULL end-to-end pipeline (Phases 2-6) on one questionnaire, in one command.",
    )
    ...
    if args.run_pipeline is not None:
        run_full_pipeline(args.run_pipeline)
        return
