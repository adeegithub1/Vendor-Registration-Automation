...
**Current status: Phase 7 — Full Pipeline Orchestration** (one command runs
the entire questionnaire → filled document pipeline end-to-end).
...
```
Vendor-AI/
├── app.py                  # Entry point (setup check + --read-docx + --extract-knowledge + --classify-questions + --generate-answers + --fill-questionnaire + --run-pipeline)
...
## Usage: Phase 7 — full pipeline (recommended for normal use)

Once your `.env` is set up and `uploads/company_documents/` has your
company's files in it, the entire pipeline runs in one command:

```
python app.py --run-pipeline path/to/vendor_questionnaire.docx
```

This calls the same five phase functions described below (`--read-docx`
through `--fill-questionnaire`), in order, automatically — nothing new is
implemented here, it's pure orchestration of the already-working
individual phases:

1. Read the questionnaire DOCX and detect every question (Phase 2)
2. Extract every company document in `uploads/company_documents/` (Phase 3)
3. Classify every question — category, answer type, likely source, priority (Phase 4)
4. Match each question to its best company document and ask the configured
   AI provider for a grounded answer (Phase 5)
5. Fill the original questionnaire and save it (Phase 6)

Result: `output/Filled_Questionnaire.docx`.

**Error handling is inherited, not reimplemented.** Each step already
tolerates its own kind of partial failure — Phase 3 skips one bad company
document and continues; Phase 5 retries a failed AI request per
`AI_MAX_RETRIES` and then records `NOT FOUND`; Phase 6 does the same for
any question it can't place — `run_full_pipeline()` in `app.py` doesn't add
any new error handling, it relies entirely on the behavior those phases
already have (and were already tested to have in earlier phases).

**When to use the individual `--read-docx` / `--extract-knowledge` / etc.
flags instead:** they still work exactly as before, unchanged — useful for
re-running just one step (e.g. you fixed a company document and only want
to redo Phase 3 onward without re-detecting questions), or for debugging
which specific step something went wrong in.

The individual phases are documented in detail below.
...
- One command (`--run-pipeline`) runs Phases 2-6 end-to-end
- Pure orchestration: zero new business logic, zero new classes — only sequences the five existing, already-tested phase functions
- No changes to `config.py`, env var names, models, logger, document processors, AI providers, prompt builder, answer generation engine, or question intelligence engine
- Error tolerance (per-document, per-AI-request, per-question) is inherited entirely from the existing modules each step already calls
- Verified: existing setup check unaffected, all five individual CLI flags still work unchanged, no import errors, no circular imports, full 5-phase run confirmed end-to-end (with a fake AI provider standing in for Gemini, for the same network-access reason described in Phase 5)
...
| 7 | Full pipeline orchestration: one command, Phases 2-6 end-to-end *(done)* |
| 8 | Broader output generation / testing |
